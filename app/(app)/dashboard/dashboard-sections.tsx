...
export async function KpiCards() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      <Card>
        <CardHeader className="pb-1 pt-4 px-4">
          <CardTitle className="text-xs text-muted-foreground font-normal uppercase tracking-wide">Előző hónap</CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
          <p className="text-2xl font-bold">{fmt(revenuePrev)}</p>
          <p className="text-xs text-muted-foreground mt-1">bevétel</p>
        </CardContent>
      </Card>
...
export async function UnbilledJobs() {
  const ctx = await getAuthContext();
  if (!ctx) return null;
  const { supabase, companyId } = ctx;

  const { data: unbilledJobs } = await supabase
    .from("jobs")
...
// --- Outstanding invoices + upcoming services ---

export async function OutstandingAndServices() {
  const ctx = await getAuthContext();
  if (!ctx) return null;
  const { supabase, companyId } = ctx;

  const now = new Date();
  const [{ data: outstandingInvoices }, { data: upcomingServices }] = await Promise.all([
    supabase.from("invoices")
      .select("id, invoice_number, gross_total, created_at, job_id, jobs(job_number, customers(name))")
      .eq("company_id", companyId)
      .eq("nav_status", "szamlazva")
